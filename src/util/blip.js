// copied from: https://github.com/jshanley/blip
// and then modified
function newBlip() {
  var blip = {};
    ...
    function loader() {
      ...
      function next() {
        if (i < names.length) {
          var name = names[i];
          i++;
          requests.push(loadSample(name, samples[name]));
        } else {
          done();
        }
      }
      function loadSample(name, url) {
        console.log('url in loadSample', url);
        return new Promise((resolve, reject) => {
          console.log('url in loadSample inside promise', url);
          var request = new XMLHttpRequest();
          request.open('GET', url, true);
          request.responseType = 'arraybuffer';
          request.addEventListener('load', loaded, false);
          console.log('request in loadSample', request);
          request.send();
          function loaded(event) {
            var req = event.target;
            var arrayBuffer = req.response;
            ctx.decodeAudioData(arrayBuffer, decoded, reject);
          }
