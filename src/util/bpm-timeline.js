export default function BPMTimeline(initialTempo) {
		...
		if (tempoMarkers.length === 0)
			...
			if (idx.length === 1) {
				...
				if (is_first(idx)){
					...
					throw new Error(`Bad beat value (${beat}) @ BPMTimeline.tempo_at_beat.`);
        }
	...
	this.tempo_at_time = function(time) {
		if (tempoMarkers.length === 0)
          ...
          throw new Error(`Bad time value (${time}) @ BPMTimeline.tempo_at_time.`);
        }
	...
	this.add_tempo_marker = function(params) {
		...
		if (formulas[params.type] !== undefined) {
			...
			throw new Error("Unsupported marker type (" + params.type + ") @ BPMTimeline.add_tempo_marker.");
		}
		...
		if (tempoMarkers.length === 0) {
			...
			if (idx.length > 1) {
				...
				throw new Error("Illegal access to a tempo marker @ BPMTimeline.add_tempo_marker.");
      }
		}
	...
	this.remove_tempo_marker = function(params) {
		if (params.endBeat !== undefined) {
			...
			if (idx.length === 1) {
				...
				throw new Error("Invalid endBeat");
      }
		} else {
